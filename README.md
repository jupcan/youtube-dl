...
- **python scripts**: *downloader.ice* given by the teacher and containing the methods to be called, aka the interface we have to use, *synctimer.py* a client invocating methods through a timer to sync downloaderscheduler objects using the synctopic created and *client.py* being a normal client with a proxy and allowing us to make use of the interface methods.  
